...
async def genmanmx(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Inicia el proceso de generación de cookie manual."""
    user_id = u.effective_user.id

    if not GATES_STATUS.get("genmanmx", True):
        await u.message.reply_text("⚠️ Genmanmx está temporalmente en mantenimiento")
    ...
    await u.message.reply_text(
        "Proceso de generación de cookie manual (Amazon MX)\n\n"
        "Envía <b>3 números de teléfono de México</b> (uno por mensaje) con el formato:\n"
        "\n+52XXXXXXXXXX\n+52XXXXXXXXXX\n+52XXXXXXXXXX\n\n"
    ...
    return WAITING_FOR_NUMBERSMX

async def receive_number_mx(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Recibe los números de teléfono del usuario."""
    user_id = u.effective_user.id
    text = u.message.text.strip()

    # Validar el formato del número +1 616 258 6985
    ...
    if len(user_data[user_id]["numeros"]) == 3:
        ...
        await u.message.reply_text(
            "⏳ Iniciando proceso de registro en Amazon...\n"
            "🔐 Cuando recibas el OTP, envíalo aquí."
        )
        asyncio.create_task(run_amazon_process(u, c, user_id))
        return WAITING_FOR_OTPMX
    else:
        return WAITING_FOR_NUMBERSMX

async def receive_otp_mx(u: Update, c: ContextTypes.DEFAULT_TYPE):
...
async def cancel_mx(u: Update, c: ContextTypes.DEFAULT_TYPE):
...
async def run_amazon_process(u: Update, c: ContextTypes.DEFAULT_TYPE, user_id):
    ...
    try:
        ...
        cookie = await registro_high_score_mx_telegram(u, c, user_id, numeros)
...
async def registro_high_score_mx_telegram(u: Update, c: ContextTypes.DEFAULT_TYPE, user_id, numeros):
    ...
    co.set_pref('intl.accept_languages', 'es-MX,es;q=0.9')
    ...
    try:
        ...
        if page.wait.ele_displayed('#whatsapp_header', timeout=60):
            time.sleep(2)
            print("Pantalla de whatsapp detectada cambiando a sms!")
            page.ele("#secondary_channel_button").click()
        ...
        elif page.wait.ele_displayed('#cvf-input-code', timeout=60):
            print("Pantalla de sms detectada, esperando OTP...")
            # Esperar a que el usuario envíe el OTP
            user_data[user_id]["event"].clear()  # Limpiar el evento
            try:
                await c.bot.send_message(
            ...
            print("❌ No se detectó pantalla de OTP. El proceso puede haber fallado.")
        ...
        session.post(
            'https://www.amazon.com.mx/a/addresses/add?ref=ya_address_book_add_post',
            data=payload_address,
            headers={**headers_base, 'referer': 'https://www.amazon.com.mx/a/addresses/add?ref=ya_address_book_add_button'},
            allow_redirects=False
        )
        # Extraer cookie
        print("[*] Calentando sesión para Akamai...")
        page.get('https://www.amazon.com.mx/alm/storefront?almBrandId=10zZeC1FBwm&ref_=nav_cs_amnow')
        time.sleep(1)

        # Petición a Wallet para forzar la actualización de at-main y sess-at-main
        res_wallet = session.get(
            'https://www.amazon.com.mx/cpe/yourpayments/wallet',
            headers={**headers_base, 'referer': 'https://www.amazon.com.mx/a/addresses'},
            timeout=15
        )
