...
async def genmanca(u: Update, c: ContextTypes.DEFAULT_TYPE):
    user_id = u.effective_user.id

    if not GATES_STATUS.get("genmanca", True):
        await u.message.reply_text("⚠️ Genmanca está temporalmente en mantenimiento")
    ...
    await u.message.reply_text(
        "Proceso de generación de cookie manual (Amazon CA 🇨🇦)\n\n"
        "Envía <b>3 números de teléfono de CANADA</b> (uno por mensaje) con el formato:\n"
        "\n+1XXXXXXXXXX\n+1XXXXXXXXXX\n+1XXXXXXXXXX\n\n"
        "Espera a que el bot confirme cada número antes de enviar el siguiente.\n"
        "0.25 CRD se descontaran una vez ingreses el tercer número", parse_mode="HTML"
    )
    return WAITING_FOR_NUMBERSCA

async def receive_number_ca(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Recibe los números de teléfono del usuario."""
    user_id = u.effective_user.id
    text = u.message.text.strip()

    # Validar el formato del número
    if not text.startswith("+1") or len(text) != 12:
        await u.message.reply_text("❌ Formato incorrecto. Usa: **+1XXXXXXXXXX**")
        return WAITING_FOR_NUMBERSCA
    ...
    if len(user_data[user_id]["numeros"]) == 3:
        ...
        await u.message.reply_text("0.25 CRD descontados")
        
        await u.message.reply_text(
            "⏳ Iniciando proceso de registro en Amazon CA 🇨🇦...\n"
            "🔐 Cuando recibas el OTP, envíalo aquí."
        )
        asyncio.create_task(run_amazon_process(u, c, user_id))
        return WAITING_FOR_OTPCA
    else:
        return WAITING_FOR_NUMBERSCA

async def receive_otp_ca(u: Update, c: ContextTypes.DEFAULT_TYPE):
...
async def cancel_ca(u: Update, c: ContextTypes.DEFAULT_TYPE):
...
async def run_amazon_process(u: Update, c: ContextTypes.DEFAULT_TYPE, user_id):
    ...
    try:
        ...
        cookie = await registro_high_score_ca_telegram(u, c, user_id, numeros)
...
async def registro_high_score_ca_telegram(u: Update, c: ContextTypes.DEFAULT_TYPE, user_id, numeros):
    ...
    co.set_pref('intl.accept_languages', 'en-CA,en')
    ...
    def bloquear_recursos_no_esenciales(page):
        page.set.blocked_urls([
            ...
            '*optimizationguide-pa.googleapis.ca*'
        ])
    ...
    try:
        ...
        url_inicio = "https://www.amazon.ca/wallet"
        page.get(url_inicio)
        ...
        if page.wait.ele_displayed('#cvf-input-code', timeout=60):
            ...
            try:
                await c.bot.send_message(
                    chat_id=user_id,
                    text="⏳ Esperando OTP... (Tiempo límite: 6 minutos)"
                )
        ...
        page.get("https://www.amazon.ca/a/addresses/add?ref=ya_address_book_add_button")
        ...
        session.post(
            'https://www.amazon.ca/a/addresses/add?ref=ya_address_book_add_post',
            data=payload_address,
            headers={**headers_base, 'referer': 'https://www.amazon.ca/a/addresses/add?ref=ya_address_book_add_button'},
            allow_redirects=False
        )
        # Extraer cookie
        print("[*] Calentando sesión para Akamai...")
        page.get('https://www.amazon.ca/haul/store?ref_=nav_cs_hul_disb')
        time.sleep(1)

        # Petición a Wallet para forzar la actualización de at-main y sess-at-main
        res_wallet = session.get(
            'https://www.amazon.ca/cpe/yourpayments/wallet',
            headers={**headers_base, 'referer': 'https://www.amazon.ca/a/addresses'},
            timeout=15
        )
