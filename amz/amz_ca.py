...
from amz.datos import obtener_numero_ca, esperar_sms, saldo_numeros, cancelar_numero
...
def registro_high_score_ca():
    ...
    proxy_server = os.getenv("AMAZON_PROXY", "p.webshare.io:10008")
    print(f"[*] Usando proxy: {proxy_server}")
    co.set_proxy(f"http://{proxy_server}")

    
    
    co.set_argument('--force-timezone=America/Toronto')
    co.set_argument('--no-first-run')
    co.set_argument('--lang=en-CA')
    co.set_argument('--disable-blink-features=AutomationControlled')
    co.set_argument('--disable-webrtc')
    co.set_argument('--disable-dev-shm-usage')
    co.set_argument('--disable-extensions')
    ...
    try:
        ...
        escritura_humana(page.ele('#ap_customer_name'), nombre)
        time.sleep(random.uniform(0.8, 1.5))
...
# Espera para que la página cargue completamente
        ...
        time.sleep(2)
        usa_option = page.ele('#address-ui-widgets-countryCode-dropdown-nativeId_223')
        ...
        escritura_humana(page.ele('#address-ui-widgets-enterAddressPhoneNumber'), numero)
        time.sleep(1)
        ...
        ### Aqui inicia la funcion de extraer la cookie de sess
        page.listen.start('wallet')
        print("Navegando a Amazon Wallet...")
        

        page.get("https://www.amazon.ca/cpe/yourpayments/wallet")
