...
from database import esbin, load_keys, get_rango # Cambia load_keys por load_extras
from engines.bins_engine import fetch_bins_engine, get_bin_dict, get_bin_dict_new, get_bin_info
...
async def bin_cmd(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    from engines.bins_engine import get_bin_dict_new
    data = await get_bin_dict_new(bin_num)
    ...
    txt = (
        ...
        "<b>━━ ━ ━ ━ ━ ━ ━ ━ ━━</b>\n"
        f"<b>Info By:</b> @{u.effective_user.username or 'User'}"
    )
...
async def bin_cmd_new(u: Update, c: ContextTypes.DEFAULT_TYPE):
    if get_rango(u.effective_user.id) < 1:
        return await u.message.reply_text("❌ Sin Acceso Premium.")
...
async def bin_master_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    # Guardamos los datos completos en cache igual que antes
    ...
    # 🗂️ --- NUEVA LÓGICA: MENÚ ALFABÉTICO ---
    # Extraemos las primeras letras únicas de los nombres reales de los bancos disponibles
    letras_disponibles = set()
    for k in sorted_keys:
        nombre_banco = data[k]['nombre'].strip()
        if nombre_banco:
            primera_letra = nombre_banco[0].upper()
            # Si empieza por un número o carácter especial, lo agrupamos en '#'
            if not primera_letra.isalpha():
                letras_disponibles.add("#")
            else:
                letras_disponibles.add(primera_letra)

    # Las ordenamos alfabéticamente (poniendo el '#' al final si existe)
    lista_letras = sorted(list(letras_disponibles), key=lambda x: (x == "#", x))

    # Construimos el teclado dinámico distribuyendo las letras en filas de hasta 4 columnas
    keyboard = []
    row = []
    for letra in lista_letras:
        # callback_data modificado para identificar que se seleccionó una letra (ej: 'let_A')
    ...
    await status.edit_text(
        f"✅ Resultados para: *{sujeto}*\n"
        f"📂 Se encontraron bancos. Selecciona una letra para desplegarlos:", 
...
def gen_logic(bin_format):
    ...
    is_amex = cc_base[:2] in ("34", "37")
    target_len = 14 if is_amex else 15  # se deja 1 dígito libre para luhn

    card = cc_base
    while len(card) < target_len:
    ...
    cvv_res = cvv if cvv else (str(random.randint(1000, 9999)) if is_amex else str(random.randint(100, 999)))
